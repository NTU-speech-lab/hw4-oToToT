...
def train_word2vec(x):
    ...
    model = word2vec.Word2Vec(x, size=500, window=5, min_count=5, workers=12, iter=25, sg=1)
    return model
...
def training(batch_size, n_epoch, lr, model_dir, train, valid, model, device, best_acc = 0):
    ...
    for epoch in range(n_epoch):
        ...
        for i, (inputs, labels) in enumerate(train):
            ...
            total_acc += (correct / len(inputs))
            total_loss += loss.item()
            print('[ Epoch{}: {}/{} ] loss:{:.3f} acc:{:.3f} '.format(
                epoch+1, i+1, t_batch, loss.item(), correct*100/len(inputs)
            ), end='\r')
...
def testing(batch_size, test_loader, model, device):
    ...
    return ret_output
def get_score(batch_size, test_loader, model, device):
    model.eval()
    ret_output = []
    with torch.no_grad():
        for inputs in test_loader:
